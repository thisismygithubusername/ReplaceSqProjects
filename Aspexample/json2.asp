...
(function () {
	...
	function str(key, holder) {
		...
		//if (key === "complex") {
		//	Response.Write("<div style=\"border: 1px solid red; padding: 10px;\">(key=" + key + ") (holder=" + typeof value + ")</div>");
		//}
...
replace(/(?:^|:|,)(?:\s*\[)+/g, ''))) {
				...
				j = eval('(' + text + ')');
				if (Object.prototype.toString.apply(j) === '[object Object]') {
					...
					if (!j.set) {
						j.set = function (k, v) {
							...
							// This black magic was based on a few hours of trial and error - BHG
							try {
								if (Object.prototype.toString.apply(v) === '[object Object]') {
									// This block is for ActiveXObjects

									/*
									if (k == 'nullTest3') {
									Response.Write("<div style=\"border: 1px solid red; padding: 10px;\">" + VBArrayIsNull(v) + "</div>\n");
									}
									*/

									// this is for an ADO Field (ie accessing a field of a recordset)
									if (Object.hasOwnProperty.call(v, "Count") == 1 && Object.hasOwnProperty.call(v, "Item")) {
										//Response.Write("<div style=\"border: 1px solid red; padding: 10px;\">IsCollection</div>\n");
										v = v.Item(1);
									} else if (Object.hasOwnProperty.call(v, "set") == 1) {
										v = v;
									} else {
										//Response.Write("<div style=\"border: 1px solid red; padding: 10px;\">IsAutomation</div>\n");
										v = "" + v;
										//v = new Date("" + v).getMilliseconds();
									}
								}

							} catch (e) {
								//Response.Write("<div style=\"border: 1px solid red; padding: 10px;\">" + Object.prototype.toString.apply(v) + "</div>\n");
								v = "" + v;
								//v = new Date("" + v).getMilliseconds();
							}

							//Response.Write("<div style=\"border: 1px solid red; padding: 10px;\">hmmm: " + Object.prototype.toString.apply(v) + "</div>\n");
	...
	if (typeof JSON.JScriptObjectToDictionary !== 'function') {
		JSON.JScriptObjectToDictionary = function (o) {
...
(function(){
	...
	/*
    if (typeof JSON.customSanitize !== 'function') {
        JSON.customSanitize = function(key, value){
			Response.Write("<div>(key=" + key + ") (value=" + value + ") (typeof=" + Object.prototype.toString.apply(value) + ")</div>\n");
			if(Object.prototype.toString.apply(value) === '[object Array]') {
				delete value.get;
				delete value.set;
				delete value['delete'];
	...
	*/
    ...
    // initial attempt to resolve JSON into nested objects, where the nested object contain usable .get, .set, .keys, and .delete
    JSON.objectReviver = function(key,j) {
    	if (Object.prototype.toString.apply(j) === '[object Object]') {
