...
<%  end if 
	...
	Dim showDetails, cSDate, cEDate, cLoc, cLoc1, cTrainer, cTime, varGen, cTG
	Dim curDate, curTime, curVisitType, curTrainer, curTrainerID, curLoc, curTG, curClassname, curClassID, curApptID
	Dim tmpRevPerVisit, tmpRev, TotRev, GTRev, tmpWeb, TotWeb, GrandTotWeb
	Dim tmpHCPaid, tmpHCComp, tmpHCNoShow, tmpHCMem
	Dim TotHCPaid, TotHCComp, TotHCNoShow, TotHCMem
	Dim GrandTotHCPaid, GrandTotHCComp, GrandTotHCNoShow, GrandTotHCMem
	Dim ss_UseAsst1, ss_UseAsst2, curAsst1, curAsst2, curRsrcName, visitTGChk, visitTGList, autoGen, pmtTGList, pmtTGChk, ap_view_all_locs
	...
	dim reportID : reportID = 0
	if RQ("reportID") <> "" AND IsNumeric(RQ("reportID")) then
		reportID = RQ("reportID")
	elseif RF("reportID") <> "" AND IsNumeric(RF("reportID")) then
	...
	If request.querystring("qGen") = "true" then
		varGen = true
		autoGen = true ' added to get data to automatically generate when coming from CSI
	...
	If request.querystring("qTime")<>"" and request.querystring("qTime")<>"0" then
		cTime = TimeValue(request.querystring("qTime"))
...
<%= js(array("mb", "MBS", "calendar" & dateFormatCode, "adm/adm_rpt_attendance", "reportFavorites", "plugins/jquery.SimpleLightBox")) %>
<%= css(array("SimpleLightBox")) %> 
			<script type="text/javascript">
			function exportReport() {
				document.frmParameter.frmExpReport.value = "true";
				document.frmParameter.frmGenReport.value = "true";
				<% iframeSubmit "frmParameter", "adm_rpt_attendance.asp" %>
			}
			</script>
...
<% if UseVersionB(TEST_REPORT_LANDING_PAGE) then %>
			...
			<% end if %>
			<span class="breadcrumb-item">&raquo;</span>
			<%= DisplayPhrase(reportPageTitlesDictionary, "Attendance") %> <span class="mainText"><a href="javascript:switchToRev();">[Include Revenue]</a></span>
			<div id="add-to-favorites">
				<span id="<%=reportID%>" class="favorite"></span><span class="text"><%= DisplayPhrase(reportPageTitlesDictionary,"Addtofavorites") %></span>
			</div>
			</div>
	<% end if %>
			...
			<% if NOT UseVersionB(TEST_REPORT_LANDING_PAGE) then %>
				...
				<tr>
					...
					<table width="100%" cellspacing="0">
						<tr>
						<td class="headText" valign="bottom"><b> <%= pp_PageTitle("Attendance") %> </b> <span class="mainText"><a href="javascript:switchToRev();">[Include Revenue]</a></span></td>
			...
			<% end if %>
					...
					<table class="mainText border4 center" cellspacing="0">
						<form name="frmParameter" action="adm_rpt_attendance.asp" method="POST">
						...
						<select name="optTime"><option value="0" <%if cTime="0" then response.write "selected" end if%>>All Start Times</option>
<%
							...
							do while NOT rsEntry.EOF
								%>
								  ...
								  <% else %>	
									    <option value="<%=rsEntry("ClassTime")%>" 
									    <%if cTime<>"0" and cTime<>"null" then
										    if TimeValue(cTime)=TimeValue(rsEntry("Classtime")) then 
											    response.write "selected" 
										    end if
									    end if%>
									    ><%=FmtTimeShort(rsEntry("Classtime"))%></option>
									<% end if %>
								<%
								rsEntry.MoveNext
							loop
...
%>
						...
						<select name="optAttTG" size=3 multiple <%showMultiSelectTitle() %>>
...
%>
						&nbsp;<%=xssStr(allHotWords(27))%>&nbsp;<%=xssStr(allHotWords(7))%>:
...
<%
					...
					<% end if 'use related TGs %>
						...
						<select name="optDisMode">
						  <option value="0" <%if showDetails="0" then response.write "selected" end if%>>By Session</option>
						...
						<br />
					<% showDateArrows("frmParameter") %> <br />						
				...
				<%end if%>
						<% savingButtons "frmParameter", "Attendance" %>
						</td>
						</tr>
					...
					<tr> 
					<td valign="top" id="attendanceNoRevGenTag" class="mainTextBig center-ch"> 
		...
		<% 
							...
							if varGen=true then 
								...
								strSQL = "SELECT  [VISIT DATA].ClientID, CLIENTS.LastName, CLIENTS.FirstName, [VISIT DATA].ClassDate, [VISIT DATA].ClassTime, " _
												& "[VISIT DATA].TrainerID, TRAINERS.TrLastName, TRAINERS.TrFirstName, TRAINERS.DisplayName, CASE WHEN NOT TRAINERS.DisplayName IS NULL THEN TRAINERS.DisplayName ELSE TRAINERS.TrLastName END AS TrnDisName, [VISIT DATA].TypeTaken, tblVisitTypes.TypeName, " _
												& "[VISIT DATA].Cancelled, [VISIT DATA].Missed, Location.LocationName, [VISIT DATA].Location, [VISIT DATA].PmtRefNo, " _
												& "[VISIT DATA].VisitRefNo, [VISIT DATA].[Value], [PAYMENT DATA].ExpDate, [PAYMENT DATA].Remaining, [PAYMENT DATA].RealRemaining, " _ 
												& "[PAYMENT DATA].Type, [PAYMENT DATA].NumClasses, [PAYMENT DATA].PaymentAmount, CLIENTS.Deleted, [VISIT DATA].TrainerID2, [VISIT DATA].TrainerID3, " _
												& "tblTypeGroup.TypeGroupID, tblTypeGroup.TypeGroup, tblClassSch.ClassID, tblClassDescriptions.ClassName, [VISIT DATA].Webscheduler, tblResources.ResourceName, tblReservation.VisitRefNo AS ApptVisitRefNo " 
								...
								if visitTGList<>"" AND visitTGList<>"0" then
									...
									strSQL = strSQL & " AND (tblTypegroup.wsResource=0) "
								end if
								if pmtTGList<>"" AND pmtTGList<>"0" then
									strSQL = strSQL & "AND PaymentTypeGroup.TypeGroupID IN (" & pmtTGList & ") "
								else
									strSQL = strSQL & " AND (PaymentTypeGroup.wsResource=0) AND (tblTypegroup.wsMedia=0) AND ([Visit Data].TrainerID IS NOT NULL)"
								end if
								' opening for days of week
								strSQL = strSQL & "AND (1 = 0 "
								if request.form("chkDaySun")="on" OR request.form("frmSubmitted")="" then
									strSQL = strSQL & "OR DATEPART(WEEKDAY, [VISIT DATA].ClassDate) = 1 "
								end if
								if request.form("chkDayMon")="on" OR request.form("frmSubmitted")="" then
									strSQL = strSQL & "OR DATEPART(WEEKDAY, [VISIT DATA].ClassDate) = 2 "
								end if
								if request.form("chkDayTue")="on" OR request.form("frmSubmitted")="" then
									strSQL = strSQL & "OR DATEPART(WEEKDAY, [VISIT DATA].ClassDate) = 3 "
								end if
								if request.form("chkDayWed")="on" OR request.form("frmSubmitted")="" then
									strSQL = strSQL & "OR DATEPART(WEEKDAY, [VISIT DATA].ClassDate) = 4 "
								end if
								if request.form("chkDayThu")="on" OR request.form("frmSubmitted")="" then
									strSQL = strSQL & "OR DATEPART(WEEKDAY, [VISIT DATA].ClassDate) = 5 "
								end if
								if request.form("chkDayFri")="on" OR request.form("frmSubmitted")="" then
									strSQL = strSQL & "OR DATEPART(WEEKDAY, [VISIT DATA].ClassDate) = 6 "
								end if
								if request.form("chkDaySat")="on" OR request.form("frmSubmitted")="" then
									strSQL = strSQL & "OR DATEPART(WEEKDAY, [VISIT DATA].ClassDate) = 7 "
								end if
								strSQL = strSQL & ") "
								if request.form("frmTagClients")<>"true" then
									if showdetails="0" then
										strSQL = strSQL & " ORDER BY tblTypeGroup.TypeGroup, [VISIT DATA].Classdate DESC, [VISIT DATA].ClassTime, tblClassSch.ClassID, " & GetTrnOrderBy() & ", Clients.LastName"
									elseif showdetails="1" then
										strSQL = strSQL & " ORDER BY [VISIT DATA].ClassDate DESC, [VISIT DATA].ClassTime, tblClassSch.ClassID, " & GetTrnOrderBy() & ", [VISIT DATA].TrainerID, tblTypeGroup.TypeGroup "
									elseif showdetails="2" then
										strSQL = strSQL & " ORDER BY " & GetTrnOrderBy() & ", tblTypeGroup.TypeGroup, [VISIT DATA].Classdate DESC, [VISIT DATA].ClassTime, tblClassSch.ClassID, Clients.LastName"
									elseif showdetails="3" then
										strSQL = strSQL & " ORDER BY [VISIT DATA].Classdate DESC, [VISIT DATA].ClassTime, tblClassSch.ClassID, " & GetTrnOrderBy() & ", Clients.LastName"
									elseif showdetails="4" then
										strSQL = strSQL & " ORDER BY tblTypeGroup.TypeGroup, " & GetTrnOrderBy() & ", [VISIT DATA].Classdate DESC, [VISIT DATA].ClassTime, tblClassSch.ClassID, Clients.LastName"
									...
									elseif showdetails="6" then
										strSQL = strSQL & " ORDER BY [VISIT DATA].Classdate, [VISIT DATA].ClassTime, tblClassSch.ClassID, tblClassDescriptions.ClassName, " & GetTrnOrderBy() 
									end if
								end if
									...
									if NOT rsEntry.EOF then			'EOF
										...
										do while NOT rsEntry.EOF
											if curTG<>rsEntry("Typegroup") and curTG="" then
												%>
												<tr>
													<td height="35" colspan="9" class="maintextbig"><strong><%=UCASE(rsEntry("TypeGroup"))%></strong></td>
												</tr>
												<%
											end if
...
<% end if %>
													<tr>
														<td><div class="right"><strong> HEAD COUNT&nbsp;</strong></div></td>
														<td class="right"><strong><%= UCASE(getHotWord(36))%>:&nbsp;</strong></td>
														<td align="left"><strong><%=tmpHCPaid%></strong></td>
														<td class="right"><strong>COMP:&nbsp;</strong></td>
														<td align="left"><strong><%=tmpHCComp%></strong></td>
														<td class="right"><strong>NO SHOW:&nbsp;</strong></td>
														<td align="left"><strong><%=tmpHCNoShow%></strong></td>
														<td class="right"><strong><%= getHotWord(54)%>:&nbsp;</strong></td>
														<td align="left"><strong><%=tmpWeb%></strong></td>
													</tr>
													<tr>
													  <td colspan="9">&nbsp;</td>
													</tr>
...
<% end if %>
													<tr class="maintextbig">
														<td><div class="right"><strong><%=UCASE(curTG)%>&nbsp;HEAD COUNT&nbsp;</strong></div></td>
														<td class="right"><strong><%= UCASE(getHotWord(36))%>:&nbsp;</strong></td>
														<td align="left"><strong><%=TotHCPaid%></strong></td>
														<td class="right"><strong>COMP:&nbsp;</strong></td>
														<td align="left"><strong><%=TotHCComp%></strong></td>
														<td class="right"><strong>NO SHOW:&nbsp;</strong></td>
														<td align="left"><strong><%=TotHCNoShow%></strong></td>
														<td class="right"><strong><%= getHotWord(54)%>:&nbsp;</strong></td>
														<td align="left"><strong><%=TotWeb%></strong></td>
													</tr>
<% if NOT request.form("frmExpReport")="true" then %>
													<tr height="1">
														  <td colspan="9" style="background-color:#666666;"><% if NOT request.form("frmExpReport")="true" then %><img src="<%= contentUrl("/asp/adm/images/trans.gif") %>" height="1" width="100%"><% end if %></td>
...
<% end if %>
												<tr>
												  <td width="20%" align="left" colspan="2"><strong><%=session("ClientHW")%> </strong></td>
												  <td width="17%" align="left" colspan="2"><strong><%=hw61%></strong></td>
												  <td width="10%" class="right"><strong><%= getHotWord(117)%> </strong></td>
												  <td width="10%" class="right"><strong>Rem </strong></td>
												  <td width="10%" class="center-ch"><strong><%=hw113%> <%= getHotWord(36)%>&nbsp;</strong></td>
												  <td width="10%" class="center-ch"><strong><%= getHotWord(172)%>&nbsp;</strong></td>
												  <td width="10%" class="center-ch"><strong>No Show&nbsp;</strong></td>
												</tr>
												<%
												tmpHCPaid=0
												tmpHCComp=0
											...
											%>
												<tr>
											<% if NOT request.form("frmExpReport")="true" then %>
												  <td colspan="2" align="left"><a href="main_info.asp?ID=<%=rsEntry("ClientID")%>&fl=true"><%=TRIM(rsEntry("LastName")) & ",&nbsp;" & TRIM(rsEntry("FirstName"))%></a></td>
											<%else %>
												  <td colspan="2" align="left"><%=TRIM(rsEntry("LastName")) & ",&nbsp;" & TRIM(rsEntry("FirstName"))%></td>
											<%end if%>
												  <td colspan="2" align="left"><%=rsEntry("TypeTaken")%></td>
												  <td class="right"><%=FmtDateShort(rsEntry("ExpDate"))%></td>
												  <td class="right">
<%
...
%>												  
												  ...
												  <td class="center-ch"><% if rsEntry("Value") then response.write "Yes" else response.write "No" end if %>&nbsp;</td>
												  <%if true=false then %>
												  <td class="center-ch"><input type="checkbox" name="chkCancelled" <%if rsEntry("Cancelled") then response.write "checked" end if %>></td>
												  <td class="center-ch"><input type="checkbox" name="chkMissed" <%if rsEntry("Missed") then response.write "checked" end if %>></td>
												  <%else %>
												  <td class="center-ch"><%if rsEntry("Cancelled") then response.write "Yes" else response.write "No" end if %></td>
												  <td class="center-ch"><%if rsEntry("Missed") then response.write "Yes" else response.write "No" end if %></td>
												  <%end if%>
												</tr>
											<%		
											rsEntry.MoveNext
...
<% end if %>
									<tr class="maintextbig">
										<td><div class="right"><strong>HEAD COUNT GRAND TOTAL&nbsp;</strong></div></td>
										<td class="right"><strong><%= UCASE(getHotWord(36))%>:&nbsp;</strong></td>
										<td align="left"><strong><%=GrandTotHCPaid%></strong></td>
										<td class="right"><strong>COMP:&nbsp;</strong></td>
										<td align="left"><strong><%=GrandTotHCComp%></strong></td>
										<td class="right"><strong>NO SHOW:&nbsp;</strong></td>
										<td align="left"><strong><%=GrandTotHCNoShow%></strong></td>
										<td class="right"><strong><%= getHotWord(54)%>:&nbsp;</strong></td>
										<td align="left"><strong><%=GrandTotWeb%></strong></td>
									</tr>
<% if NOT request.form("frmExpReport")="true" then %>
									<tr height="2">
										  <td colspan="9" style="background-color:#666666;"><% if NOT request.form("frmExpReport")="true" then %><img src="<%= contentUrl("/asp/adm/images/trans.gif") %>" height="1" width="100%"><% end if %></td>
											...
											if curDate<>rsEntry("ClassDate") then	'if this is a new day then write the header cells
												if curDate<> "" then	'if this isn't the first record total cells
													if curDate<>rsEntry("ClassDate") or curTime<>rsEntry("ClassTime") or curTG<>rsEntry("Typegroup") or curVisitType<>rsEntry("TypeName") or curLoc<>rsEntry("LocationName") or curTrainerID<>CLNG(rsEntry("TrainerID")) or curClassID<>rsEntry("ClassID") then	'if this is a new class then write the header cells
														...
														%>
															<tr>
															  <td align="left"><%=FmtTimeShort(curTime)%></td>
															  <td align="left"><%=curTG%></td>
															  <td align="left"><%=curTrainer%></td>
															  <td align="left">
<% if NOT request.form("frmExpReport")="true" then %>
															<% if curClassID<>"" then %>
...
<% end if %>
															  ...
															  <td class="right"><%=tmpHCPaid%></td>
															  <td class="right"><%=tmpHCComp%></td>
															  <td class="right"><%=tmpHCNoShow%></td>
															  <td class="right"><%=tmpWeb%></td>
															  <td class="right"><%=tmpHCMem%></td>
															</tr>
														<%
...
<% end if %>
													<tr>
														<td colspan="4"><div class="right"><strong><%=FmtDateLong(curDate)%> <%= getHotWord(22)%>:</strong></div></td>
														<td class="right"><strong><%=TotHCPaid%></strong></td>
														<td class="right"><strong><%=TotHCComp%></strong></td>
														<td class="right"><strong><%=TotHCNoShow%></strong></td>
														<td class="right"><strong><%=TotWeb%></strong></td>
														<td class="right"><strong><%=TotHCMem%></strong></td>
													</tr>
													<tr>
													  <td colspan="9">&nbsp;</td>
													</tr>
												...
												%>
												<tr>
												  <td colspan="9" class="blackHeader" align="left"><strong><%=FmtDateLong(rsEntry("ClassDate"))%>&nbsp;</strong></td>
												</tr>
<% if NOT request.form("frmExpReport")="true" then %>
												<tr height="2">
													  <td colspan="9" style="background-color:#666666;"><% if NOT request.form("frmExpReport")="true" then %><img src="<%= contentUrl("/asp/adm/images/trans.gif") %>" height="1" width="100%"><% end if %></td>
...
<% end if %>
												<tr>
												  <td width="8%" align="left"><strong><%= getHotWord(58)%> </strong></td>
												  <td width="14%" align="left"><strong><%=hw7%> </strong></td>
												  <td width="12%" align="Left"><strong><%=hw6%></strong></td>
												  <td width="12%" align="Left"><strong>Visit Type </strong></td>
												  <td width="10%" class="right"><strong><%= getHotWord(36)%>&nbsp;<%=session("ClientHW")%>s</strong></td>
												  <td width="10%" class="right"><strong>Comp <%=session("ClientHW")%>s</strong></td>
												  <td width="10%" class="right"><strong>No Shows</strong></td>
												  <td width="7%" class="right"><strong><%= getHotWord(54)%></strong></td>
												  <td width="7%" class="right"><strong><%= getHotWord(220)%></strong></td>
												</tr>
<% if NOT request.form("frmExpReport")="true" then %>
												<tr height="1">
													<td colspan="9" style="background-color:#666666;"><% if NOT request.form("frmExpReport")="true" then %><img src="<%= contentUrl("/asp/adm/images/trans.gif") %>" height="1" width="100%"><% end if %></td>
...
<% end if %>
									<tr>
									  <td colspan="4" class="maintextbig"><div class="right"><strong>GRAND TOTAL&nbsp;</strong></div></td>
									  <td class="right"><strong><%=GrandTotHCPaid%></strong></td>
									  <td class="right"><strong><%=GrandTotHCComp%></strong></td>
									  <td class="right"><strong><%=GrandTotHCNoShow%></strong></td>
									  <td class="right"><strong><%=GrandTotWeb%></strong></td>
									  <td class="right"><strong><%=GrandTotHCMem%></strong></td>
									</tr>
<% if NOT request.form("frmExpReport")="true" then %>
									<tr height="2">
										  <td colspan="9" style="background-color:#666666;"><% if NOT request.form("frmExpReport")="true" then %><img src="<%= contentUrl("/asp/adm/images/trans.gif") %>" height="1" width="100%"><% end if %></td>
...
<% if NOT request.form("frmExpReport")="true" then %>
															<tr height="1">
																<td colspan="1" style="background-color:#FFFFFF;"><% if NOT request.form("frmExpReport")="true" then %><img src="<%= contentUrl("/asp/adm/images/trans.gif") %>" height="1" width="100%"><% end if %></td>
																<td colspan="9" style="background-color:#666666;"><% if NOT request.form("frmExpReport")="true" then %><img src="<%= contentUrl("/asp/adm/images/trans.gif") %>" height="1" width="100%"><% end if %></td>
															</tr>
<% end if %>
															<tr>
																<td>&nbsp;</td>
																<td><div align="left"><strong><%=FmtDateTime(curDate)%>&nbsp;<%=FmtTimeShort(curTime)%>&nbsp;<%If isnull(curClassName) then response.write  LEFT(curVisitType,18) else response.write LEFT(curClassName,18) end if%></strong></div></td>
																<td class="right"><strong><%= UCASE(getHotWord(36))%>:&nbsp;</strong></td>
																<td align="left"><strong><%=tmpHCPaid%></strong></td>
																<td class="right"><strong>COMP:&nbsp;</strong></td>
																<td align="left"><strong><%=tmpHCComp%></strong></td>
...
<% if NOT request.form("frmExpReport")="true" then %>
													<tr height="1">
														  <td colspan="10" style="background-color:#666666;"><% if NOT request.form("frmExpReport")="true" then %><img src="<%= contentUrl("/asp/adm/images/trans.gif") %>" height="1" width="100%"><% end if %></td>
													</tr>
<% end if %>
													<tr class="right">
														<td>&nbsp;</td>
														<td nowrap align="left"><strong><%=UCASE(Replace(curTrainer,"&nbsp;"," "))%> - HEAD COUNT</strong></td>
														<td><strong><%= UCASE(getHotWord(36))%>:&nbsp;</strong></td>
														<td align="left"><strong><%=TotHCPaid%></strong></td>
												...
												%>
												<tr class="blackHeader">
												  <td colspan="10" align="left"><strong><%=FmtTrnNameNew(rsEntry, false)%>&nbsp;</strong></td>
												</tr>
<% if NOT request.form("frmExpReport")="true" then %>
													<tr height="2">
														  <td colspan="10" style="background-color:#666666;"><% if NOT request.form("frmExpReport")="true" then %><img src="<%= contentUrl("/asp/adm/images/trans.gif") %>" height="1" width="100%"><% end if %></td>
...
<% end if %>
												<tr>
												  <td width="5%">&nbsp;</td>
												  <td colspan="2" width="20%" align="left"><strong><%=session("ClientHW")%> </strong></td>
												  <td colspan="3" width="17%" align="left"><strong><%=hw61%></strong></td>
												  <td width="10%" class="right"><strong><%= getHotWord(117)%> </strong></td>
												  <td width="10%" class="center-ch"><strong>Rem </strong></td>
												  <td width="15%" class="center-ch"><strong><%= getHotWord(172)%>&nbsp;</strong></td>
												  <td width="10%" class="center-ch"><strong>No Show&nbsp;</strong></td>
												</tr>
												<%
												tmpHCPaid=0
											...
											%>
												<tr align="left">
												  <td>&nbsp;</td>
											<% if NOT request.form("frmExpReport")="true" then %>
												  <td colspan="2"><a href="main_info.asp?ID=<%=rsEntry("ClientID")%>&fl=true"><%=TRIM(rsEntry("LastName")) & ",&nbsp;" & TRIM(rsEntry("FirstName"))%></a></td>
											<%else %>
												  <td colspan="2"><%=TRIM(rsEntry("LastName")) & ",&nbsp;" & TRIM(rsEntry("FirstName"))%></td>
											<%end if%>
												  <td colspan="3"><%=rsEntry("TypeTaken")%></td>
												  <td class="right"><%=FmtDateShort(rsEntry("ExpDate"))%></td>
												  <td class="center-ch">
<%
											if rsEntry("RealRemaining")=rsEntry("Remaining") then
												if rsEntry("Type") = 0 then
													response.write "<span style=""color:#990000;"">" & rsEntry("RealRemaining")*-1 & "&nbsp;owed</b></span>"
												else
													response.write rsEntry("RealRemaining")
												end if
...
<% end if %>
											<tr>
												  <td>&nbsp;</td>
												<td><div align="left"><strong><%=UCASE(Replace(curTrainer,"&nbsp;"," "))%> - HEAD COUNT</strong></div></td>
												<td class="right"><strong><%= UCASE(getHotWord(36))%>:&nbsp;</strong></td>
												<td align="left"><strong><%=TotHCPaid%></strong></td>
												<td class="right"><strong>COMP:&nbsp;</strong></td>
												<td align="left"><strong><%=TotHCComp%></strong></td>
...
<% end if %>
									<tr class="maintextbig">
										<td>&nbsp;</td>
										<td><div align="left"><strong>HEAD COUNT GRAND TOTAL</strong></div></td>
										<td class="right"><strong><%= UCASE(getHotWord(36))%>:&nbsp;</strong></td>
										<td align="left"><strong><%=GrandTotHCPaid%></strong></td>
										<td class="right"><strong>COMP:&nbsp;</strong></td>
										<td align="left"><strong><%=GrandTotHCComp%></strong></td>
...
<% end if %>
															<tr>
																<td>&nbsp;</td>
																<td><div align="left"><strong><%=FmtTimeShort(curTime)%>&nbsp;<%=curTrainer%>&nbsp;<%If isnull(curClassName) then response.write LEFT(curVisitType,12) else response.write LEFT(curClassName,12) end if%></strong></div></td>
																<td class="right"><strong><%= UCASE(getHotWord(36))%>:&nbsp;</strong></td>
																<td align="left"><strong><%=tmpHCPaid%></strong></td>
																<td class="right"><strong>COMP:&nbsp;</strong></td>
																<td align="left"><strong><%=tmpHCComp%></strong></td>
...
<% end if %>
													<tr class="right">
														<td>&nbsp;</td>
														<td nowrap align="left"><strong><%=FmtDateLong(curDate)%> - HEAD COUNT</strong></td>
														<td><strong><%= UCASE(getHotWord(36))%>:&nbsp;</strong></td>
														<td align="left"><strong><%=TotHCPaid%></strong></td>
														<td><strong>COMP:&nbsp;</strong></td>
														<td align="left"><strong><%=TotHCComp%></strong></td>
												...
												%>
												<tr>
												  <td colspan="10" class="blackHeader" align="left"><strong><%=FmtDateLong(rsEntry("ClassDate"))%>&nbsp;</strong></td>
												</tr>
<% if NOT request.form("frmExpReport")="true" then %>
													<tr height="2">
														  <td colspan="10" style="background-color:#666666;"><% if NOT request.form("frmExpReport")="true" then %><img src="<%= contentUrl("/asp/adm/images/trans.gif") %>" height="1" width="100%"><% end if %></td>
...
<% end if %>
											<tr>
												<td>&nbsp;</td>
												<td><div align="left"><strong><%=FmtDateLong(curDate)%> - HEAD COUNT</strong></div></td>
												<td class="right"><strong><%= UCASE(getHotWord(36))%>:&nbsp;</strong></td>
												<td align="left"><strong><%=TotHCPaid%></strong></td>
												<td class="right"><strong>COMP:&nbsp;</strong></td>
												<td align="left"><strong><%=TotHCComp%></strong></td>
...
<% if NOT request.form("frmExpReport")="true" then %>
															<tr height="1">
													  		<td style="background-color:#FFFFFF;"><% if NOT request.form("frmExpReport")="true" then %><img src="<%= contentUrl("/asp/adm/images/trans.gif") %>" height="1" width="100%"><% end if %></td>
															<td colspan="9" style="background-color:#666666;"><% if NOT request.form("frmExpReport")="true" then %><img src="<%= contentUrl("/asp/adm/images/trans.gif") %>" height="1" width="100%"><% end if %></td>
															</tr>
<% end if %>
															<tr>
																<td>&nbsp;</td>
																<td><div align="left"><strong><%=FmtDateTime(curDate)%>&nbsp;<%=FmtTimeShort(curTime)%>&nbsp;<%=curTrainer%>&nbsp;<%If isnull(curClassName) then response.write LEFT(curVisitType,12) else response.write LEFT(curClassName,12) end if%></strong></div></td>
																<td class="right"><strong><%= UCASE(getHotWord(36))%>:&nbsp;</strong></td>
																<td align="left"><strong><%=tmpHCPaid%></strong></td>
																<td class="right"><strong>COMP:&nbsp;</strong></td>
																<td align="left"><strong><%=tmpHCComp%></strong></td>
...
<% end if %>
													<tr class="right">
														<td>&nbsp;</td>
														<td align="left"><strong><%=UCASE(curTG)%> - HEAD COUNT</strong></td>
														<td><strong><%= UCASE(getHotWord(36))%>:&nbsp;</strong></td>
														<td align="left"><strong><%=TotHCPaid%></strong></td>
														<td><strong>COMP:&nbsp;</strong></td>
														<td align="left"><strong><%=TotHCComp%></strong></td>
												...
												%>
												<tr>
												  <td colspan="10" class="blackHeader" align="left"><strong><%=UCASE(rsEntry("Typegroup"))%>&nbsp;</strong></td>
												</tr>
<% if NOT request.form("frmExpReport")="true" then %>
													<tr height="2">
														  <td colspan="10" style="background-color:#666666;"><% if NOT request.form("frmExpReport")="true" then %><img src="<%= contentUrl("/asp/adm/images/trans.gif") %>" height="1" width="100%"><% end if %></td>
...
<% end if %>
											<tr>
												<td>&nbsp;</td>
												<td><div align="left"><strong><%=UCASE(curTG)%> - HEAD COUNT</strong></div></td>
												<td class="right"><strong><%= UCASE(getHotWord(36))%>:&nbsp;</strong></td>
												<td align="left"><strong><%=totHCPaid%></strong></td>
												<td class="right"><strong>COMP:&nbsp;</strong></td>
												<td align="left"><strong><%=totHCComp%></strong></td>
...
<% end if %>
															<tr>
																<td>&nbsp;</td>
																<td><div align="left"><strong><%=FmtDateTime(curDate)%>&nbsp;<%=FmtTimeShort(curTime)%>&nbsp;<%=curTG%>&nbsp;<%=curTrainer%></strong></div></td>
																<td class="right"><strong><%= UCASE(getHotWord(36))%>:&nbsp;</strong></td>
																<td align="left"><strong><%=tmpHCPaid%></strong></td>
																<td class="right"><strong>COMP:&nbsp;</strong></td>
																<td align="left"><strong><%=tmpHCComp%></strong></td>
...
<% end if %>
													<tr class="right">
														<td>&nbsp;</td>
														<td align="left"><strong><%=UCASE(curVisitType)%> - HEAD COUNT</strong></td>
														<td><strong><%= UCASE(getHotWord(36))%>:&nbsp;</strong></td>
														<td align="left"><strong><%=TotHCPaid%></strong></td>
														<td><strong>COMP:&nbsp;</strong></td>
														<td align="left"><strong><%=TotHCComp%></strong></td>
												...
												%>
												<tr>
												  <td colspan="10" class="blackHeader" align="left"><strong><%=UCASE(rsEntry("TypeName"))%>&nbsp;</strong></td>
												</tr>
<% if NOT request.form("frmExpReport")="true" then %>
													<tr height="2">
														  <td colspan="10" style="background-color:#666666;"><% if NOT request.form("frmExpReport")="true" then %><img src="<%= contentUrl("/asp/adm/images/trans.gif") %>" height="1" width="100%"><% end if %></td>
...
<% end if %>
											<tr>
												<td>&nbsp;</td>
												<td><div align="left"><strong><%=UCASE(curVisitType)%> - HEAD COUNT</strong></div></td>
												<td class="right"><strong><%= UCASE(getHotWord(36))%>:&nbsp;</strong></td>
												<td align="left"><strong><%=totHCPaid%></strong></td>
												<td class="right"><strong>COMP:&nbsp;</strong></td>
												<td align="left"><strong><%=totHCComp%></strong></td>
...
<% end if %>
								...
								'*********************  ROLL SHEET  *****************************
									...
									if NOT rsEntry.EOF then			'EOF
									%>
									<table class="mainText" width="650"  cellspacing="0">
										<tr>
										<td colspan="10" class="mainTextBig"><b><%=WeekDayName(WeekDay(cSDate))%> - <%=MonthName(Month(cSDate))%>&nbsp;<%=Day(cSDate)%>,&nbsp;<%=Year(cSDate)%></b></td>
										</tr>
<% if NOT request.form("frmExpReport")="true" then %>
										<tr height="2">
											  <td colspan="10" style="background-color:#666666;"><% if NOT request.form("frmExpReport")="true" then %><img src="<%= contentUrl("/asp/adm/images/trans.gif") %>" height="1" width="100%"><% end if %></td>
...
<% end if %>
										<tr valign="bottom">
										  <%If cSDate<>cEDate then%><td width="60" align="left"><strong><%= getHotWord(57)%> </strong></td>
										  <%end if%>
										  <td width="50" align="left"><strong><%= getHotWord(58)%> </strong></td>
										  <td width="100" align="Left"><strong>Class Name </strong></td>
										  <%if session("useResrcResv") then%>
										  <td width="60" align="Left"><strong><%=hw0%></strong></td>
										  <% end if %>
										  <td width="80" align="Left"><strong>
									      <%=hw6%></strong></td>
										  <%If ss_UseAsst1 then%><td width="80" align="left"><strong>
										    <%=hw13%> </strong></td><%end if%>
										  <%If ss_UseAsst2 then%><td width="80" align="left"><strong>
										    <%=hw15%> </strong></td><%end if%>
										  <td width="50" class="center-ch"><strong>#Enrolled</strong></td>
										  <td width="50" class="center-ch"><strong>#<%= getHotWord(36)%></strong></td>
										  <td width="50" class="center-ch"><strong>#Comps&nbsp;</strong></td>
										  <td class="center-ch"><strong>Head Count</strong></td>
									  </tr>
<% if NOT request.form("frmExpReport")="true" then %>
										<tr height="1">
											<td colspan=<%If (ss_UseAsst1 and ss_UseAsst2) then response.write "10" else if (ss_UseAsst1 or ss_UseAsst2) then response.write "9" else response.write "8" end if%> style="background-color:#666666;"><% if NOT request.form("frmExpReport")="true" then %><img src="<%= contentUrl("/asp/adm/images/trans.gif") %>" height="1" width="100%"><% end if %></td>
										</tr>
<% end if %>
										<%
										do while NOT rsEntry.EOF
											if curDate<>rsEntry("ClassDate") or curTime<>rsEntry("ClassTime") or curClassName<>rsEntry("ClassName") or curLoc<>rsEntry("LocationName") or curTrainerID<>CLNG(rsEntry("TrainerID")) then	'if this is a new class then write the header cells
												...
												%>
													<tr height="30">
													  <%If cSDate<>cEDate then%><td align="left"><%=FmtDateShort(curDate)%></td><%end if%>
													  <td align="left"><%=FmtTimeShort(curTime)%></td>
													  <td align="left"><%=curClassName%>&nbsp;</td>
													  <%if session("useResrcResv") then%>
													  <td align="Left"><%=curRsrcName%>&nbsp;</td>
													  <% end if %>
													  <td align="left"><%=curTrainer%></td>
													  <%If ss_UseAsst1 then%><td align="left"><%If isnull(curAsst1) then Response.write "" else response.write FmtTrnName(curAsst1) end if%></td><%end if%>
													  <%If ss_UseAsst2 then%><td align="left"><%If isnull(curAsst2) then Response.write "" else response.write FmtTrnName(curAsst2) end if%></td><%end if%>
													  <td class="center-ch"></td>
													  <td class="center-ch"><%=tmpHCPaid%></td>
													  <td class="center-ch"><%=tmpHCComp%></td>
													  <td class="center-ch"></td>
													</tr>
													<tr height="30">
													  <%If cSDate<>cEDate then%><td align="left">&nbsp;</td><%end if%>
													  <td align="left">&nbsp;</td>
													  <td align="left">&nbsp;</td>
													  <%if session("useResrcResv") then%>
													  <td align="left">&nbsp;</td>
													  ...
													  <%If ss_UseAsst1 then%><td align="left">_____________________________&nbsp;&nbsp;</td><%end if%>
													  <%If ss_UseAsst2 then%><td align="left">_____________________________</td><%end if%>
													  <td class="center-ch">________</td>
													  <td class="center-ch">&nbsp;</td>
													  <td class="center-ch">&nbsp;</td>
													  <td class="center-ch">________</td>
													</tr>
...
<% if NOT request.form("frmExpReport")="true" then %>
										<tr height="2">
											<td colspan=<%If (ss_UseAsst1 and ss_UseAsst2) then response.write "11" else if (ss_UseAsst1 or ss_UseAsst2) then response.write "10" else response.write "9" end if%> style="background-color:#666666;"><% if NOT request.form("frmExpReport")="true" then %><img src="<%= contentUrl("/asp/adm/images/trans.gif") %>" height="1" width="100%"><% end if %></td>
										</tr>
<% end if %>
										<tr>
										  <td colspan="8">&nbsp;</td>
