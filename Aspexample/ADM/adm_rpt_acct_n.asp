...
<%= js(array("mb", "adm/adm_rpt_acct_n")) %>
...
<tr> 
     <td class="center-ch" valign="top" height="100%" width="100%"> <br />
        <table cellspacing="0" width="90%" height="100%">
          <tr> 
            <td class="headText" align="left"><b><%=session("ClientHW")%>s without Logins </b></td>
          </tr>
          <tr>
            <td valign="top" class="mainText right">&nbsp;</td>
          </tr>
          <tr> 
            <td valign="top" class="mainText center-ch"> 
              <table class="mainText" width="95%" cellspacing="0">
                <tr > 
                  <td  colspan="2" valign="top" class="mainText center-ch">
		<form name="frmParameter" action="adm_rpt_acct_n.asp" method="POST">
		...
		<%
			...
			'if request.form("frmTagClients")="true" then
				...
				strSQLTag = strSQLTag & "FROM Clients WHERE LoginName IS NULL AND [Deleted]=0" 
...
<%
		...
		strSQL = strSQL & "WHERE LoginName IS NULL AND [Deleted]=0 "
        ...
        intCount = 0
        %>
<b>Click on a <%=session("ClientHW")%>'s name to edit their Contact Information </b><br />
...
<table class="mainText" cellspacing="0">
  <tr style="background-color:<%=session("pageColor2")%>;"><td colspan=5><img height="2" width="100%" src="<%= contentUrl("/asp/adm/images/trans.gif") %>"></td></tr>
